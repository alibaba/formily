...
**Formily** 内置 **ArrayCards**, **ArrayTable** 作为官方自增列表组件，两者使用方式完全一致，主要差异在UI上。下面用 **ArrayTable** 举例（**ArrayCards**同理）， 代码如下：
...
底层均是基于 [ArrayList](https://github.com/alibaba/formily/blob/master/packages/react-shared-components/src/ArrayList.tsx) 的，不过承载的容器组件分别是 **Table** 和 **Card**, DEMO如下：
    ...
    <SchemaForm actions={actions} components={{ ArrayCards, Input }}>
        <Field
          ...
          x-component-props={{
            ...
            renderAddition: () => '+add',
            renderRemove: (idx) => {
              return <FormSpy selector={[['onFieldValueChange',`userList.${idx}.username`]]}>
