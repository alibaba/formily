...
test('eachChildren eacher',()=>{
  ...
  expect(shallowChilds).toEqual([{ node: vf1, path: FormPath.getPath(vf1.state.path)}])
  ...
  expect(deepChilds).toEqual([
    ...
    { node: vf1GrandChildren, path: FormPath.getPath(vf1GrandChildren.state.path)},
  ])
})
...
test('eachChildren path eacher',()=>{
  ...
  // deep mode: recursion = true 
  const deepChilds = []
  graph.eachChildren("a", (node, path) => {
    deepChilds.push({ node, path })
  }, true)
  expect(deepChilds).toEqual([
    { node: vf1Children, path: FormPath.getPath(vf1Children.state.path)},
...
test('eachChildren path selector eacher',()=>{
  ...
  // shallow mode: recursion = false 
  const shallowChilds = []
  graph.eachChildren("a", "a.a.a", (node, path) => {
    shallowChilds.push({ node, path })
  }, true)
  ...
  // deep mode: recursion = true 
  const deepChilds = []
  graph.eachChildren("a", "a.a", (node, path) => {
    deepChilds.push({ node, path })
  }, true)
  expect(deepChilds).toEqual([
    { node: vf1Children, path: FormPath.getPath(vf1Children.state.path)},
