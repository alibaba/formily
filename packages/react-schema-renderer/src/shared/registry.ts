...
export function registerVirtualFields(object: ISchemaFormRegistry['virtualFields']) {
  each<ISchemaFormRegistry['virtualFields'], ISchemaVirtualFieldComponent>(
    object,
    (component, key) => {
      registerVirtualBox(key, component)
    }
  )
}
